...
params <- SetPeakParam(ppm = 22, noise = p23$noise, value_of_prefilter = p23$value_of_prefilter, prefilter = p23$prefilter, min_peakwidth = 2, max_peakwidth = 8, snthresh = 100, mzdiff = 0.1)
...
test_data_xsannotate <- groupFWHM(test_data_xsannotate, perfwhm = 3)
...
querySPLASH <- get_splashscores(msp_object = list(test_data_msp))
...
query_thirdblocks <- get_blocks(splashscores = querySPLASH, blocknr = 3)
database_thirdblocks <- get_blocks(splashscores = full_mona_SPLASHES, blocknr = 3)
SPLASH_matches <- match_nines(query_blocks = query_thirdblocks, database_blocks = database_thirdblocks)
similarity_scores <- similarities_thirdblocks(msp_query = test_data_msp, database = mona_msp, nine_matches = SPLASH_matches)
...
params2 <- SetPeakParam(ppm = 22, noise = p24$noise, value_of_prefilter = p24$value_of_prefilter, prefilter = p24$prefilter, min_peakwidth = 2, max_peakwidth = 8, snthresh = 100, mzdiff = 0.1)
...
blabla1 <- PerformPeakPicking(blabla, param = updateRawSpectraParam(params))
...
match_nines <- function(query_blocks, database_blocks) {
  ...
  total_indexes <- vector(mode = 'list', length = length(query_blocks))
  total_indexes <- lapply(query_blocks, function(x) {
    query_nines <- str_locate(x, '9')
    indexes_p_sample <- vector(mode = 'list', length = length(query_nines[, 1]))
    indexes_p_sample <- lapply(seq_along(query_nines[, 1]), function(y) {
      indexes <- vector(mode = 'numeric', length = length(db_nines[, 1]))
      indexes <- sapply(seq_along(db_nines[, 1]), function(z) ifelse(test = query_nines[,1][[y]] == db_nines[, 1][[z]], yes = z, no = 0))
...
get_splashscores <- function(msp_object) {
  ...
  splashscoresquery <- sapply(seq_along(msp_object), function(x) {
