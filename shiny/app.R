...
ui <- dashboardPage(
  ...
  sidebar = dashboardSidebar(
    ...
    sidebarMenu(collapsed = T,
      ...
      menuItem('Peak annotation', tabName = 'annotation', icon = icon('search')),
    ...
    tabItems(
      ...
      tabItem('peakpicking',
              fluidRow(
                column(
                  width = 4,
                  box(id = 'testje', width = 6, collapsible = T, collapsed = F,
                  ...
                  div(id = 'align_param_box',
                      box(width = 6, collapsible = T, collapsed = T,
                          style = "font-size:11px;",
                  ...
                  box(
                    ...
                    collapsible = T, 
                    collapsed = T,
                    numericInput(inputId = 'perfwhm', label = 'Full width half maximum', value = 3, min = 0, max = 100, step = 1),
                ...
                column(
                  ...
                  box(
                    ...
                    DTOutput(outputId = 'foundcompoundstable')
                  ))
                )),
      tabItem('annotation',
              fluidRow(
                box(
                  title = 'Annotationtabitem'
                )
              )),
      tabItem('clustering',
              fluidRow(
...
server <- function(input, output, session){
  ...
  observeEvent(input$peakdetectrun, {
    ...
    if (length(input$data_input$datapath) == 1) {
      ...
      mSet_xsannotate <- groupFWHM(mSet_xsannotate, perfwhm = 3)
      mSet_msp <- to.msp(object = mSet_xsannotate, file = NULL, settings = NULL, ndigit = 3, minfeat = 1, minintens = 0, intensity = "maxo", secs2mins = F)
      for (i in mSet_msp) {
        print(i)
      }
